...
function getErrorDetails(code: VoteErrorCode): {
  title: string;
  description: string;
} {
  switch (code) {
    case "VOTING_CLOSED":
      return {
        ...
        description:
          "The voting period has ended or has not started yet. Please check with the electoral committee for voting schedules.",
      };
    case "NOT_REGISTERED":
      return {
        title: "Not Registered",
        description:
          "Your state code is not found in the CDS members registry. Only registered members can vote. Please contact the electoral committee if you believe this is an error.",
      };
    case "COMMITTEE_MEMBER":
      return {
        title: "Committee Members Cannot Vote",
        description:
          "As a member of the electoral committee, you are not eligible to vote. This ensures fair and unbiased election proceedings.",
      };
    case "INELIGIBLE":
      return {
        title: "Account Ineligible",
        description:
          "Your account has been marked as ineligible to vote. This may be due to policy violations or administrative actions. Contact the electoral committee for details.",
      };
    case "ALREADY_VOTED":
      return {
        title: "Already Voted",
        description:
          "You have already cast your vote for this position. Each member can only vote once per position to ensure fair election results.",
      };
    default:
      return {
        title: "Cannot Vote",
        description:
          "An error occurred while processing your vote. Please try again or contact support.",
      };
  }
}
...
export function VotingInterface({
  ...
  const verifyVoter = async (isPreviewCheck = false) => {
    ...
    if (memberError || !member) {
      setVoteError({
        ...
        description:
          "Your state code is not registered in the CDS members list.",
      });
      setVerifying(false);
      setStep("verify");
    ...
    if (!member.eligible) {
      setVoteError({
        code: "INELIGIBLE",
        message:
          member.ineligible_reason || "Your account is not eligible to vote",
        description:
          "Please contact the electoral committee for more information.",
      });
      setVerifying(false);
      setStep("verify");
      return;
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onBlur={() => verifyVoter(true)} // 👈 AUTO CHECK HERE
              onKeyDown={(e) => e.key === "Enter" && verifyVoter(false)}
          ...
          <div className="pt-4 border-t">
            <p className="text-xs text-center text-muted-foreground">
              🔒 Your identity is protected through anonymization. Only your
              hashed state code is stored with your vote.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="max-w-md mx-auto">
        <CardContent className="pt-8 pb-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Your votes have been recorded securely and anonymously. Results will
            be announced by the electoral committee.
          </p>
